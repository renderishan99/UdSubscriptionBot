...
def start_handler(message):
    ...
    if len(text) > 1:
        ...
        if ch_data:
            ...
            bot.send_message(message.chat.id, f"Swaagat hai!\n\nChannel: *{ch_data['name']}*\n\nNiche se apna plan chunein:", reply_markup=markup, parse_mode="Markdown")
            return

    if user_id == ADMIN_ID:
        bot.send_message(message.chat.id, "Admin Panel Active!\n/add - Naya Channel ya Price Edit karein\n/channels - List Channels")

@bot.message_handler(commands=['add'], func=lambda m: m.from_user.id == ADMIN_ID)
def add_channel_start(message):
    msg = bot.send_message(ADMIN_ID, "Jis channel ko add/edit karna hai, uska koi bhi message yahan FORWARD karein.")
...
def get_plans(message):
    if message.forward_from_chat:
        ...
        msg = bot.send_message(ADMIN_ID, 
            f"Channel: {ch_name}\n\nAb plans likhein is format mein:\n`Minutes:Price, Minutes:Price` \n\n"
            "*Example for Testing (1 & 2 mins):*\n`1:5, 2:10` \n\n"
            "*Example for Real (1 Month & 3 Months):*\n`43200:199, 129600:499`", parse_mode="Markdown")
...
def finalize_channel(message, ch_id, ch_name):
    try:
        # Format parsing: "1:5, 2:10" -> {"1": "5", "2": "10"}
...
def user_pays(call):
    ...
    bot.send_photo(call.message.chat.id, qr_url, 
                   caption=f"Plan: {mins} Minutes\nAmount: ₹{price}\nUPI: `{UPI_ID}`\n\nPayment ke baad 'I Have Paid' dabayein.", 
...
def admin_notify(call):
    ...
    bot.send_message(ADMIN_ID, f"🔔 *Payment Verification!*\nUser: {user.first_name} (@{user.username})\nChannel: {ch_data['name']}\nPlan: {mins} Mins\nPrice: ₹{price}", 
                     reply_markup=markup, parse_mode="Markdown")
    
    # Notify User
    u_markup = InlineKeyboardMarkup().add(InlineKeyboardButton("📞 Contact Admin", url=f"https://t.me/{CONTACT_USERNAME}"))
    bot.send_message(call.message.chat.id, "Aapki request admin ko bhej di gayi hai. Agar 5 min tak link na mile toh niche click karein.", reply_markup=u_markup)
...
def approve_now(call):
    ...
    try:
        ...
        expiry = datetime.now() + timedelta(minutes=int(mins))
        
        users_col.update_one({"user_id": int(u_id), "channel_id": int(ch_id)}, 
                             {"$set": {"expiry": expiry.timestamp()}}, upsert=True)
        
        bot.send_message(int(u_id), f"✅ Payment Approved!\n\nAapka link ye raha: {link.invite_link}\n\nYe link sirf {mins} minute tak valid rahega (Testing ke liye).")
        bot.edit_message_text(f"Approved for {mins} mins!", call.message.chat.id, call.message.message_id)
...
def kick_logic():
    ...
    for u in expired:
        try:
            ...
            markup = InlineKeyboardMarkup().add(InlineKeyboardButton("Dobara Join Karein", url=f"https://t.me/{bot.get_me().username}"))
            bot.send_message(u['user_id'], "⚠️ Aapka subscription khatam ho gaya hai. Dobara join karne ke liye niche click karein.", reply_markup=markup)
...
if __name__ == '__main__':
    ...
    scheduler.add_job(kick_logic, 'interval', minutes=1) # Har 1 minute me check karega
    scheduler.start()
